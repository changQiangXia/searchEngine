    ...
    def temporal_distribution(
        ...
        if query:
            ...
            if self.engine.index:
                results = [
                    {"metadata": m, "index": i} for i, m in enumerate(self.engine.index.metadata)
                ]
            else:
                results = []
    ...
    def find_similar_time_periods(
        ...
        for ref_bucket in ref_buckets:
            ...
            for tgt_bucket in tgt_buckets:
                if (
                    tgt_bucket.start <= ref_bucket.end
                    and tgt_bucket.end >= ref_bucket.start
                    and tgt_bucket.count > 0
                ):
