...
from nexus_mind.infrastructure.compute.optimizer import (
    BatchConfig, DynamicBatcher, PerformanceMonitor,
    optimize_for_3080ti, optimize_for_4090,
)
...
class NexusEngine:
    ...
    def __init__(
        ...
        self.index: Optional[FAISSBackend] = None
        ...
        from nexus_mind.plugins.base import PluginRegistry, PluginLoader
    ...
    def search(
        ...
        if self.index is None:
            raise RuntimeError(
                "No index loaded. Build an index first with index_images()."
            )
    ...
    def explore_chain(
        ...
        if Path(start).exists():
            return chain.explore_from_image(
                start, steps=steps, strategy=strategy, top_k=top_k
            )
        else:
            return chain.explore_from_text(
                start, steps=steps, strategy=strategy, top_k=top_k
            )
