...
def get_engine(workspace: Optional[str] = None) -> NexusEngine:
...
def search(
    ...
    diverse: bool = typer.Option(
        False, "--diverse", "-d", help="Use MMR for diverse results"
    ),
):
    """Search the index."""
    engine = get_engine(workspace)
...
def negative(
    ...
    alpha: float = typer.Option(
        0.7, "--alpha", "-a", help="Negative weight (0-1)"
    ),
):
...
def workspace(
    ...
    create: Optional[str] = typer.Option(
        None, "--create", "-c", help="Create new workspace"
    ),
    info: Optional[str] = typer.Option(
        None, "--info", "-i", help="Show workspace info"
    ),
):
    """Manage workspaces."""
    ...
    if info:
        ...
        if index_path.exists():
            console.print(f"Index: [green]Exists[/green]")
            # Load and show stats
            try:
...
@app.command()
def blend(
    concepts: List[str] = typer.Argument(..., help="Concepts with optional weights (e.g., 'vintage:0.8' 'neon:0.2')"),
    workspace: Optional[str] = typer.Option(
        None, "--workspace", "-w", help="Workspace directory"
    ),
